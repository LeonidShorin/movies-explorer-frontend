...
function Profile({onEditProfile, isEditProfile, onFormSubmit}) {
  ...
  function Button() {
    if (isEditProfile === true) {
      return (
        <button className='profile-form__button profile-form__button_type_submit' type={'submit'}>Сохранить</button>)
    } else {
      return (
        <button className='profile-form__button profile-form__button_type_edit' type={'button'} onClick={onEditProfile}>Редактировать</button>)
    }
  }
  ...
  return (<div className='profile'>
      <div className='section section_type_profile'>
        <form className='profile-form'
              ...
              noValidate={true}>
          <h2 className='profile-form__heading'> Привет, Леонид! </h2>
